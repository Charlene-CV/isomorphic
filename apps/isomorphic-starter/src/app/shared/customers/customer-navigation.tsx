...
export default function CustomerNav({ props }: any) {
  ...
  const items = [
    {
      label: 'People',
      value: routes.people(uuid),
    },
    {
      label: 'Addresses',
      value: routes.addresses(uuid),
    },
  ];
          ...
          >
            {items.map((menu, index) => (
              <Link
                ...
                key={`menu-${index}`}
                className={cn(
                  'group relative cursor-pointer whitespace-nowrap py-2.5 font-medium text-gray-500 before:absolute before:bottom-0 before:left-0 before:z-[1] before:h-0.5  before:bg-[#a5a234] before:transition-all hover:text-gray-900',
                  menu.value.toLowerCase() === pathname
                    ? 'before:visible before:w-full before:opacity-100'
                    : 'before:invisible before:w-0 before:opacity-50'
                )}
              >
                <Text
                  as="span"
